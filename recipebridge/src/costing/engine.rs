...
//! - Sub-recipes recompute per encounter with a cloned visited set (no
//!   memoization), exactly like the TS recursion.
...
impl<'a> Engine<'a> {
    ...
    fn own_trio(&self, row: &WCostingRow, visited: &HashSet<String>) -> Trio {
        let Some(amount) = &row.amount else {
            return Trio::all_err(format!("ingredient {} has no amounts", row.id));
        };
        let measure = amount.to_measure();
        match row.kind {
            WRowKind::Recipe => match self.sub_recipe_pairs(&row.target_id, visited) {
                Some(pairs) => self.measures(&measure, &make_graph(&pairs)),
                None => Trio::all_err(format!(
                    "sub-recipe {} could not be costed",
                    row.target_id
                )),
            },
            WRowKind::Ingredient => {
                self.measures(&measure, self.ctx_for(&row.target_id).graph())
            }
        }
    }
    ...
    fn sub_recipe_pairs(
        ...
        let totals = self.cost_recipe(sub, &v, false);
        ...
        for t in &self.targets {
            if let Some(n) = totals.nutrients.iter().find(|n| n.code == t.code) {
                pairs.push((yield_measure.clone(), Measure::new(&t.unit, n.value)));
            }
        }
        Some(pairs)
    }
    ...
    fn resolve_row(
        ...
        let own = needs_own.then(|| self.own_trio(row, visited));

        // One estimate trio per distinct gram value (in practice: one).
        let mut est: Vec<(f64, Trio)> = Vec::new();
        for s in plan.sources() {
            ...
            #[allow(clippy::float_cmp)] // exact same expression both times
            if !est.iter().any(|(g, _)| *g == grams) {
                est.push((grams, self.estimated_trio(grams, row)));
            }
        }
        ...
        let own_ref = || own.as_ref().expect("own trio precomputed for own-* sources");
    ...
    pub fn cost_recipe(
        ...
        let n = planned.len();
        let mut outcomes: Vec<Option<(Trio, Option<Trio>, Option<f64>)>> =
        ...
        for (idx, p) in planned.iter().enumerate() {
            ...
            let (trio, own) = self.resolve_row(p.row, &p.plan, 0.0, visited);
        ...
        for idx in deferred {
            let p = &planned[idx];
            let (trio, own) = self.resolve_row(p.row, &p.plan, basis_grams, visited);
            fold(&trio, &p.row.name);
