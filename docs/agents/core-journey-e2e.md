...
| Monarch statement transaction                                 | `pnpm test:e2e:headless:wardrobe` uploads [`synthetic-monarch-wardrobe.csv`](../../apps/web/tests/e2e/fixtures/synthetic-monarch-wardrobe.csv) at `/statement-rows/import`, explicitly classifies the purchase charge, holds a card-payment row, repeats the upload, and checks allocation and both durable source rows | Synthetic financial account because CSV cannot establish account identity | Native CSV review and late-arriving statement allocation still need their own checks and worklist                                                      |
