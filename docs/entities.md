...
export default defineEntity({
  ...
  route: {
    basePath: "examples",
    // A create contract plus a capture intent gives this route a dialog.
    // `createOverride: "page"` links a hand-written `examples.new.tsx`;
    // `createOverride: null` hides the generic create entry point.
    // List and detail modules are generated by default. `listOverride: null`
    // keeps a list hand-written; `detailOverride: { query }` supplies a
    // generic detail page for an entity outside the kernel detail roster.
  },
  table: "Example",
...
web and the native renderers draw from. The declaration uses `*Override`
names where a value replaces a compiler default; the generated manifest uses
...
`images` are derived from capabilities and never declared. `list.viewOverrides` names
...
schema carries `view` when there is more than one. `list.actionOverrides`
replaces the delete action inferred from the delete capability; `list.links`
names the header links. `list.timeline` names the date fields and
...
first one, supplied the start. `edit.sectionOverrides` (`[{ id, title, fields,
...
remain unchanged. Cards use the declared title and `list.shelfSubtitleOverride`
...
Related display images come from a ranked singular outgoing relation to an
Image or image-storing subject. Direct Image references rank first. An explicit
`capabilities.images.displaySourceOverrides` array replaces that ranking;
`[]` opts out. The compiler does not borrow through incoming activity or
...
sortable-column list) and optional `groupable`/`computed` subsets.
`defaultOverride` replaces `createdAt` (or the first sort field), and
`directionOverride` replaces the field-kind-based direction.
...
The list and detail route modules for a routed entity are generated by default
(`routes/_authenticated/<basePath>.index.tsx`
...
image, cookbook, the read-only Run) declares `detailOverride: { query }`, and the
generator rejects `detailOverride: null` except for `recipe` (its route still renders
