...
| T7 Specialist workbenches | In progress | Calendar and physical-workflow recovery, recommendation recovery, and the project-detail phone agenda are implemented; remaining specialist batches and full rendered-state review are pending |
...
| 2026-08-26 | T7 | Calendar recovery | Pass for implementation | Range-query failures now suppress empty calendar/agenda projections and expose an accessible phone-safe Retry action; focused Calendar coverage passes |
| 2026-08-26 | T7 | Physical workflow error truth | Pass for implementation | Photo Pass preserves scanner/scope/resume state while distinguishing lookup/tree failures from empty/completed work; Inventory Session blocks recount/save when tree or expected-inventory snapshots fail; 25 focused tests pass |
| 2026-08-26 | T7 | Recommendation recovery | Pass for implementation | Placement, tag propagation, product relatedness, and duplicate-product panels distinguish failures from unavailable/stale-index states and expose phone-safe recovery without flattening their specialist structure |
| 2026-08-26 | T7 | Project-detail phone agenda | Pass for implementation | The full split-pane Gantt remains at `md` and wider; phones receive a chronological ruled task/subproject agenda from the same subtree data with truthful ranges/status/trade, 44px links, and dated/unscheduled empty states; 4 focused tests pass |
