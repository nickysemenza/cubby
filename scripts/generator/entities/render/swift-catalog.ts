...
export const SWIFT_MANIFEST_TYPES =
...
// entity vocabulary only currently hits "enum" (an EntityFieldKind) and
// "default" (EntityTimelineMode); a set so a future kind named after another
// keyword is expected with the backticks Swift requires.
 ...
 */
export const parseSwiftEnums = (source: string): Map<string, SwiftEnum> => {
...
const fieldJSON = (
  ...
  const renderer = <Value extends string>(
    name: string,
    ...
    value === null || value === undefined
      ? null
      : vocabulary.member(name, value, `${where}.${surface}`);
  return {
    key: field.key,
    ...
    reference:
      field.reference === null
        ? null
        : {
            entity: entityKey(field.reference.entity, `${where}.reference`),
            multiple: field.reference.multiple,
            scope: field.reference.scope.map(
              ({ sourceField, targetField }) => ({
                sourceField,
                targetField,
              }),
            ),
            filters: field.reference.filters.map(({ field: name, values }) => ({
              field: name,
              values: [...values],
            })),
          },
    ...
    controlSection: field.control?.section ?? null,
    controlWidth: field.control?.width ?? null,
    controlOptions: options(field.control?.options ?? null),
    placeholder: field.control?.placeholder ?? null,
    initial: field.control?.initial ?? null,
    ...
    showInList: field.display.list,
    showInDetail: field.display.detail,
    detailOrder: field.display.detailOrder ?? null,
    listOrder: field.display.listOrder ?? null,
    listHidden: field.display.listHidden,
    width: field.display.width ?? null,
    format: field.display.format ?? null,
    listRenderer: renderer(
      "ListRendererID",
      field.display.renderer?.list,
      "display.renderer.list",
    ),
    detailRenderer: renderer(
      "DetailRendererID",
      field.display.renderer?.detail,
      "display.renderer.detail",
    ),
    mobileSlot: field.display.mobile?.slot ?? null,
    mobilePriority: field.display.mobile?.priority ?? null,
