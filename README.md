...
- **Manufacturer spelling snapped on create** — `create_products` now resolves `manufacturer` to the established spelling already used among live Products, closing the drift that let variant spellings accumulate; `update_products` deliberately does not auto-snap.
