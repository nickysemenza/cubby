...
extension RecountSession {
    ...
    public func annotation(forScanned raw: String) -> ShelfAnnotation? {
        ...
        if let label = CubbyLabel(trimmed), label.key == .location {
            let id = LocationCode(label.code)
            let name = tree?[id]?.name ?? label.code
            return ShelfAnnotation(
                title: name, detail: id == bin.id ? "This bin" : "Another bin",
                tone: id == bin.id ? .verified : .unexpected)
        }
        // Text that is neither a label nor a barcode is not a scan; there is nothing to annotate.
        guard CubbyLabel(trimmed) != nil || ScanCodes.gtin14(trimmed) != nil else { return nil }
        guard let state = row(matchingScanned: trimmed) else {
...
extension ScanSession {
    ...
    public func annotation(forScanned raw: String) -> ShelfAnnotation? {
        guard let seen = scanned[ScanCodes.key(forScanned: raw)] else { return nil }
        switch seen.status {
        case .pending:
