...
public enum BinPlan {
    public static func plan(scanned: LocationCode, anchor: LocationCode, in tree: LocationTree) -> BinVerdict {
        guard let target = tree[scanned], let anchorNode = tree[anchor] else {
            return .refuse(reason: .unknownLabel, message: "\(scanned.rawValue) isn't a location in this tree.")
        }
        if target.id == anchor {
        ...
        guard let parent = tree.parent(of: target.id) else {
            return .refuse(reason: .root, message: "\(target.name) holds the whole house — it can't sit on a shelf.")
        }
        if parent.id == anchor {
            return .confirm
        }
        if tree.isDescendant(anchor, of: target.id) {
            return .refuse(reason: .ancestor, message: "\(target.name) contains \(anchorNode.name) — it can't move inside it.")
        }
        return .adopt(AdoptableBin(id: target.id, name: target.name, type: target.type, currentParentName: parent.name))
    }
}
