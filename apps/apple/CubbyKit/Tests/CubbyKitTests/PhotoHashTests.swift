...
@Suite("LibraryHashCache")
struct LibraryHashCacheTests {
    @Test func modificationDateAndRevisionArePartOfTheKey() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let url = directory.appendingPathComponent("hashes.json")
        let cache = LibraryHashCache(fileURL: url)
        let date = Date(timeIntervalSince1970: 100)
        let hash = PerceptualHash64(value: 42)
        try await cache.store(hash, forLocalIdentifier: "asset", modificationDate: date)
        #expect(await cache.hash(forLocalIdentifier: "asset", modificationDate: date) == hash)
        #expect(
            await cache.hash(
                forLocalIdentifier: "asset", modificationDate: date.addingTimeInterval(1)) == nil)
        #expect(
            await cache.hash(
                forLocalIdentifier: "asset", modificationDate: date, revision: 2) == nil)
        try await cache.flush()

        let reloaded = LibraryHashCache(fileURL: url)
        #expect(await reloaded.hash(forLocalIdentifier: "asset", modificationDate: date) == hash)
    }

    @Test func writesAutomaticallyAtSixtyFourChanges() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let url = directory.appendingPathComponent("hashes.json")
        let cache = LibraryHashCache(fileURL: url)
        for index in 0..<64 {
            try await cache.store(
                PerceptualHash64(value: UInt64(index)),
                forLocalIdentifier: "asset-\(index)",
                modificationDate: nil)
        }
        #expect(FileManager.default.fileExists(atPath: url.path(percentEncoded: false)))
        let reloaded = LibraryHashCache(fileURL: url)
        #expect(
            await reloaded.hash(forLocalIdentifier: "asset-63", modificationDate: nil)
                == PerceptualHash64(value: 63))
    }

    @Test func replacesPriorAssetVersionAndPrunesDeletedAssetsAcrossReload() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let url = directory.appendingPathComponent("hashes.json")
        let cache = LibraryHashCache(fileURL: url)
        let oldDate = Date(timeIntervalSince1970: 100)
        let currentDate = Date(timeIntervalSince1970: 200)
        let oldHash = PerceptualHash64(value: 1)
        let currentHash = PerceptualHash64(value: 2)
        let deletedHash = PerceptualHash64(value: 3)

        try await cache.store(oldHash, forLocalIdentifier: "kept", modificationDate: oldDate)
        try await cache.store(currentHash, forLocalIdentifier: "kept", modificationDate: currentDate)
        try await cache.store(deletedHash, forLocalIdentifier: "deleted", modificationDate: nil)

        #expect(await cache.hash(forLocalIdentifier: "kept", modificationDate: oldDate) == nil)
        #expect(await cache.hash(forLocalIdentifier: "kept", modificationDate: currentDate) == currentHash)
        #expect(try await cache.prune(to: ["kept"]) == 1)
        try await cache.flush()

        let reloaded = LibraryHashCache(fileURL: url)
        #expect(await reloaded.hash(forLocalIdentifier: "kept", modificationDate: oldDate) == nil)
        #expect(
            await reloaded.hash(forLocalIdentifier: "kept", modificationDate: currentDate)
                == currentHash)
        #expect(await reloaded.hash(forLocalIdentifier: "deleted", modificationDate: nil) == nil)
    }
}
