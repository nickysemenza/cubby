...
final class StubLookupService: LookupService, Sendable {
    ...
    let catalog: Mutex<[String: UPCLookup]>
    let calls = Mutex<[String]>([])

    init(products: [String: [EntityRow]] = [:], catalog: [String: UPCLookup] = [:]) {
...
struct CodeLookupTests {
    ...
    private static let bulbs = UPCLookup(
        upc: "00012345678905", name: "LED bulbs", manufacturer: nil, category: nil, priceDollars: nil,
        imageURL: nil, source: "upcitemdb", cached: false)
    ...
    @Test func barcodeMatchingProductsReturnsThem() async throws {
        ...
        let service = StubLookupService(products: ["012345678905": rows])
        let outcome = try await CodeLookup(service: service).resolve("012345678905")

        #expect(outcome == .products(rows, code: .barcode("012345678905")))
        #expect(service.calls.withLock { $0 } == ["products:012345678905"])
    }
    ...
    @Test func isbnIsNormalizedBeforeTheServerSeesIt() async throws {
        ...
        #expect(outcome == .products(rows, code: .isbn("09780306406157")))
    }
    ...
    @Test func unknownBarcodeCarriesTheCatalogAnswerWhenThereIsOne() async throws {
        ...
        #expect(outcome == .unknownCode(.barcode("00012345678905"), catalog: Self.bulbs))
    ...
    @Test func aCatalogMissIsNotAFailure() async throws {
        ...
        #expect(outcome == .unknownCode(.barcode("012345678905"), catalog: nil))
    }
