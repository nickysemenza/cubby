...
struct MappingTests {
    ...
    @Test func mealNutritionPreservesMacroEstimateStatesAndFoodSources() throws {
        ...
        #expect(food.amount == Amount(value: 1, unit: "serving"))
        #expect(food.amountDescription == "1 serving")
        let ingredient = try #require(person.foods.dropFirst().first)
        #expect(ingredient.source == .ingredient(id: "meal-food-preview-2", ingredientID: "ING-2345"))
        #expect(ingredient.amount == Amount(value: 1, unit: "cup"))
        #expect(ingredient.grams == nil)
        let recipe = try #require(person.foods.last)
        #expect(recipe.amount == Amount(value: 0.25, unit: "batch"))
        #expect(recipe.grams == nil)
