...
- **A trailing slack spacer absorbs all surplus width**, so columns never grow past their declared `w-*` — which is why a sparse table can show several hundred px of dead space next to columns that are clipping. Pass **`fillWidth`** to `<RTable>` to spend that surplus on the columns instead; it distributes proportionally, so each `w-*` becomes a *share*. On by default for the sparse lists (search, recipes, ingredients, inventory, locations, vendors, images, purchases); off where the declared widths already fill the container (products, expenses), since there's no surplus to spend and the prop would be inert. **New list page: check for dead space at a wide window before adding it** — it's a measurement, not a default.
