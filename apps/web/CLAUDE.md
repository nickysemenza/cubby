...
- **Leftover width goes to the columns, proportionally.** The trailing gutter cell is pinned to `w-0` so it can't hoard the surplus (left unsized it's the only auto cell under `table-fixed`, and it swallowed everything — that's what put several hundred px of dead space next to columns that were clipping). Each `w-*` therefore reads as a *share* as much as a size. This is unconditional; there's no opt-out, because no table wanted one — where the declared widths already fill the container there's simply no surplus to spend.
- **Don't make a column auto-width to let it "flex".** Under `table-fixed` an unsized column absorbs the squeeze in the narrow direction too, all the way to zero — measured on search, the name column rendered **0px wide at a 900px viewport**, where sized columns instead overflow the container and scroll. `min-width` on a cell can't save it (fixed layout ignores min/max-width on cells; only `width` counts). A floor would have to go on the `<table>` element, which is why the leftover width is spread proportionally across the sized columns rather than handed to one column.
...
**The shared tRPC client cannot fetch on the server.** `getUrl()` in `root-provider.tsx` resolves to `http://localhost:${PORT}/api/trpc` when there is no `window` — a self-fetch that carries no session cookie in dev, and on CF Workers never reaches the app at all. There the edge answers with the plain-text body `error code: 1003`, so the batch link's JSON parse throws `Unexpected token 'e', "error code: 1003" is not valid JSON` into the route error boundary. It only fires on a **direct load**; client-side navigation skips SSR, which is how a broken route hides in review (#703, `/locations/arrange`).

Two consequences, both load-bearing:

- **Route loaders `void queryClient.prefetchQuery(...)`, never `await ensureQueryData(...)`** for a plain tRPC query. Awaiting throws the self-fetch failure into the boundary; components gate on their own loading skeleton instead.
- **A route whose component tree suspends on a tRPC query sets `ssr: false`.** `useSuspenseQuery` runs during the server render, so it takes the same path — and unlike a decorative fan-out it can't be hydration-gated behind `useHydrated()`, because the suspense read is what produces the data. This is why 19 routes are client-only — every `$shortcode` detail route except `products.$shortcode`, plus `cookbooks.index` and `locations.arrange`. `products.$shortcode` is the deliberate exception, and it earns it by not self-fetching at all (below).

**To genuinely server-render authenticated data, don't self-fetch — go in-process.** `products.$shortcode` is the worked example: `createRequestDomainClient()` (`server/api/request-caller.ts`) builds a tRPC client over `unstable_localLink`, reusing the incoming headers and the normal context/auth/superjson without an HTTP hop; `lib/product-detail-server.ts` wraps it in `createServerFn` so the client build compiles an RPC stub instead of bundling the server router; the loader then swaps **only** `queryFn` under `import.meta.env.SSR`, keeping the tRPC query key identical so the result dehydrates into the normal cache and the component's `useSuspenseQuery` hydrates warm. Budget the CPU before adopting it on a heavy query — the loader now runs inside the page's own Worker invocation, and per the root CLAUDE.md the workerd clock is frozen during synchronous CPU, so a `Server-Timing` figure is loader latency, not CPU.
