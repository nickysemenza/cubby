...
- **`ssr: false` is now a cost decision, not a correctness one.** A route that suspends on a tRPC query renders fine on the server; the question is only whether you want to pay for it. Reach for `ssr: false` when a route dehydrates a large payload — `/locations/arrange` is the one that does (844 KB of HTML against 84 KB client-only), and it carries a comment saying so.
- **The Start plugin's strip is what keeps the server router out of the browser.** Don't import `trpc-transport-server` (or anything under `~/server`) from shared client code except through the `.server()` branch of an isomorphic fn. Verify with a `build:cf` and a grep of `dist/client/assets` for `localLink` / `drizzle-orm` — both must be absent there and present in `dist/server/assets`.
