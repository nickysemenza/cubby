...
// unattachable from iOS). Other create/update/delete are opt-in, listed
// with a reason alongside the RPC ids it calls in native-operations.json.
// The generator config is written from that so the allowlist and the config
// cannot drift.
...
for (const [index, id] of nativeOperations.entries()) {
  ...
  if (isAutomaticResourceOperation(id))
    throw new Error(
      `native-operations.json lists ${id}; list/get and image-attachable update operations are automatic`,
    );
...
# Every automatic resources.*.list/get operation, every gallery entity's
# resources.*.update, plus the RPC and opt-in resources.*.create/update/delete
# ids in native-operations.json;
...
const attachCases = entities
  .filter(
    ([entity, actions]) =>
      ...
      updateBodyHas(entity, "pendingImageIds"),
  )
  .map(
    ([entity]) => `        case .${swiftCase(entity)}:
...
const orderCases = entities
  .filter(
    ([entity, actions]) =>
      actions.has("update") &&
      generatedOperationIds.has(`resources.${entity}.update`) &&
      updateBodyHas(entity, "imageOrder"),
  )
  .map(
    ([entity]) => `        case .${swiftCase(entity)}:
            _ = try await client.${swiftMethod(entity, "update")}(
...
// bodies `EntityOperations.swift` switches on, and the RPC ids in
// native-operations.json — so the page reads the same facts the Swift emit
// does instead of a second hand-kept list. RPC ids attach to an entity by
// their first segment (`product.lookupUpc` → product); a workflow domain that
// is not an entity key (`garden.*`, `agent.ask`) belongs to no entity here.
const nativeCoverage = Object.fromEntries(
  entityKeys.map((entity) => {
    ...
    return [
      ...
      {
        httpActions: actions,
        imageAttach: canUpdate && updateBodyHas(entity, "pendingImageIds"),
        imageOrder: canUpdate && updateBodyHas(entity, "imageOrder"),
...
emitGenerated(
  new URL(
    "../src/lib/generated/entity-native-coverage.gen.ts",
    import.meta.url,
  ),
  `// Generated by apps/web/scripts/generate-http-openapi.ts — do not edit.
// Regenerate with \`pnpm --filter @cubby/web generate:http-api\`; \`pnpm check\` fails when stale.
