...
// automatically (the generic Browse screens need them all); create/update/
// delete are opt-in, listed with a reason alongside the RPC ids it calls in
// native-operations.json. The generator config is written from that so the
// allowlist and the config cannot drift.
...
for (const [index, id] of nativeOperations.entries()) {
  ...
  if (isAutomaticResourceOperation(id))
    throw new Error(
      `native-operations.json lists ${id}; list/get operations are automatic`,
    );
...
# Every automatic resources.*.list/get operation, plus the RPC and opt-in
# resources.*.create/update/delete ids in native-operations.json;
