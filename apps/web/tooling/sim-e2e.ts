...
const artifacts = path.join(repoRoot, "artifacts/sim-e2e", simName);
...
async function assertNativeEdit(productId: string): Promise<void> {
  ...
  try {
    ...
    if (result.rows[0]?.name !== SIM_PRODUCT_UPDATED_NAME) {
      throw new Error(
        `Native edit did not reach ${simName}: ${JSON.stringify(result.rows)}`,
      );
    }
    console.log(`[sim-e2e] Native edit verified in ${simName}`);
  } finally {
    await checkPool.end();
  }
}
...
async function run(
  ...
  if (interrupted)
    throw new Error(`Simulator E2E interrupted by ${interrupted}`);
  appendFileSync(
...
async function main(): Promise<void> {
  ...
  const cleanup = async (): Promise<Error[]> => {
    ...
    if (created) {
      try {
        ...
        else console.log(`[sim-e2e] Dropped ${simName}`);
      } catch (error) {
        errors.push(error instanceof Error ? error : new Error(String(error)));
      }
    }
  ...
  try {
    ...
    created = true;
    console.log(`[sim-e2e] Disposable database ${simName}`);
    ...
    console.log(
      `[sim-e2e] Workerd at ${url.origin}; seeded product ${productId}`,
    );
    ...
    console.error(`[sim-e2e] Failure artifacts: ${artifacts}`);
  } finally {
    const cleanupErrors = await cleanup();
    if (cleanupErrors.length > 0)
      failure = new AggregateError(
        failure === undefined ? cleanupErrors : [failure, ...cleanupErrors],
        `Simulator E2E failed with cleanup errors for ${simName}`,
      );
  }
