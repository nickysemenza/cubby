export function resolveE2EWorkers(
  ...
  // harness, and a database, so they contend for the same finite CPU. CI sets
  // CUBBY_E2E_WORKERS per lane in ci.yaml (two per chromium shard); the
  // default of one only covers other non-macOS hosts.
