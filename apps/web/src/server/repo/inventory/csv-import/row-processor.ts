...
import { syncProductPrice } from "~/server/repo/product";
...
async function executeProductOperations(
  ...
  if (row.price != null && row.price !== undefined) {
    // Wrap in transaction to ensure price column is synced from unit mappings
    await withTransaction(db, async (tx) => {
      ...
      await syncProductPrice(tx, productData.id);
    });
  }
