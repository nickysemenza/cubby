...
export async function previewDeleteLedgerParties(
  db: Database,
  ids: LedgerPartyId[],
) {
  const [attributions, accounts, outgoing, incoming, households] =
    await Promise.all([
      countByTarget(
        getDb(db),
        expenseAttribution,
      ...
      countByTarget(getDb(db), ledgerTransfer, ledgerTransfer.fromPartyId, ids),
      countByTarget(getDb(db), ledgerTransfer, ledgerTransfer.toPartyId, ids),
      unwrapDb(db)
        .select({ id: ledgerParty.id })
        .from(ledgerParty)
        .where(
          and(
            inArray(ledgerParty.id, ids),
  ...
  return {
    blockers: present([
      impact({
        disposition:
          LEDGER_PARTY_DELETE_EDGE_POLICY["ExpenseAttribution.ledgerPartyId"],
      ...
      impact({
        disposition:
          LEDGER_PARTY_DELETE_EDGE_POLICY["FinancialAccount.ledgerPartyId"],
      ...
      impact({
        disposition:
          LEDGER_PARTY_DELETE_EDGE_POLICY["LedgerTransfer.fromPartyId"],
      ...
      impact({
        disposition:
          LEDGER_PARTY_DELETE_EDGE_POLICY["LedgerTransfer.toPartyId"],
      ...
      impact({
        disposition: {
          code: "protected-household",
          effect: "block",
          description: "The singleton household party is protected.",
        },
        label: "protected household party",
        byTargetId: Object.fromEntries(households.map((row) => [row.id, 1])),
      }),
    ]),
    changes: [],
    sideEffects: [],
