...
export function registerSearchTools(server: McpServer) {
  registerMcpTool(server, {
    name: "global_search",
    description:
      "Fuzzy name search across every indexed entity in one call — products, inventory entries, locations, recipes, ingredients, cookbooks, meals, and the household tracker (projects, tasks, purchases). Ranked hybrid of lexical (exact/substring/trigram) and semantic matching; pass mode: 'lexical' to skip the embedding call for a faster, name-only pass. Each hit returns entityType + id, so this is the way to turn a name into an ID before calling get_*/update_* tools. For entity-to-entity matching (which product does this purchase refer to?) use find_similar_entities instead.",
  ...
  registerRouterTool(server, {
    ...
    description:
      "Find the entities whose stored embedding is closest to one seed entity's, along an allowlisted direction: purchase_to_product (which product does this purchase line refer to?), product_to_product / ingredient_to_ingredient (duplicate hunting), recipe_to_recipe (related recipes). Pass the pair key plus the seed's id; returns each neighbour with its cosine `similarity`, nearest first, and echoes the resolved `source` ref. " +
