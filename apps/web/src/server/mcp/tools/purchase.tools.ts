...
export function registerPurchaseTools(server: McpServer) {
  ...
  registerEntityCrudToolset(server, {
    ...
    operations: { delete: false },
    descriptions: {
      list: "List vendor purchases. A `purchase` is one vendor order, receipt, or deliberately separate purchase event, NOT an Expense or card charge. Each row returns vendor identity, order/date fields, Expense totals (the spend), the literal vendor `statedTotal`, classified documents, financial reconciliation, and computed dataQuality. Related Expenses, FinancialTransactions, Products, and Projects each support an exact public-id filter, a has/none presence filter, and terminal-name text search; different related paths combine with AND. Start a completeness audit with dataStatus=needs_data; narrow with dataGap (one or several Purchase/Product checks). Product gaps roll up with the PRD- targetId. Sorted newest purchase first.",
