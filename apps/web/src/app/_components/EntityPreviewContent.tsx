...
type PreviewStat = { label: string; value: ReactNode; caption?: string };

const costStat = (cost: Parameters<typeof formatEstimate>[0]) => ({
  label: "Cost",
  value: formatEstimate(cost, formatCurrency),
});

/**
 * Rollups a detail read computes rather than declares as fields; they are
 * the numbers a hover over these three entities is usually asking for.
 */
type RollupStats = {
  [E in ManifestPreviewEntity]?: (
    data: EntityDetailByEntity[E],
  ) => PreviewStat[];
};

const ROLLUP_STATS: RollupStats = {
  recipe: (data) =>
    data.totals
      ? [
          costStat(data.totals.cost),
          ...KEY_NUTRIENT_KEYS.map((key) => {
...
export function manifestPreviewCard<E extends ManifestPreviewEntity>(
  ...
  const thumbUrl = hero.images
    ...
    : undefined;
  // SAFETY: `ROLLUP_STATS` is keyed by the same entity `data` was read for.
  const rollup = ROLLUP_STATS[entity] as
    | ((record: EntityDetailByEntity[E]) => PreviewStat[])
    | undefined;
  const stats = [
    ...entityPreviewFacts(entity, data),
    ...(rollup?.(data) ?? []),
  ];
