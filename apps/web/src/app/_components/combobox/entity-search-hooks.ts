...
export interface UseEntitySearchConfig<TId extends string, TRow, TDetail> {
  ...
  /**
   * "dialog" renders `EntitySearchCreateDialog` (the generic
   * `EntityEditDialog` capture request) on create; "none" leaves creation
   * entirely to `useOnCreateNew` (vendor mints its own row on save, with no
   * dialog in the picker at all).
   */
  createNew: "dialog" | "none";
