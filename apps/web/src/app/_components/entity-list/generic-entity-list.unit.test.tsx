...
/**
 * Entities whose index route is generated (`route.list: true`) yet sit
 * outside the kernel list roster, read off the generated route modules
 * themselves so a new one cannot be forgotten.
 */
const generatedIndexRoutes = import.meta.glob<string>(
  "../../../routes/_authenticated/*.index.tsx",
  { eager: true, query: "?raw", import: "default" },
);
const kernelListRoster = new Set<string>(listEntities);
const routedEntityKeys = new Set<string>(browserRoutedEntities);
const isBrowserRoutedEntity = (key: string): key is BrowserRoutedEntity =>
  routedEntityKeys.has(key);
const offRosterGeneratedIndexEntities = Object.values(generatedIndexRoutes)
  .filter((source) => source.startsWith("// Generated by"))
  .flatMap(
    (source) => /listPage\(\{\s*entity: "([^"]+)"/u.exec(source)?.[1] ?? [],
  )
  .filter((entity) => !kernelListRoster.has(entity))
  .filter(isBrowserRoutedEntity)
  .sort();
...
describe("resolveListView", () => {
  ...
  it.each(offRosterGeneratedIndexEntities)(
    ...
    (entity) => {
      const override = listOverrides[entity];
      expect(override, `${entity} needs a list-columns override`).toBeDefined();
      if (!override || override.mode === "client") return;
      harness = createBrowserTestHarness();
      const { result } = renderHook(
        // SAFETY: a source-mode override reads neither slot prop before
        // returning its parts; only whether it supplies a source is checked.
        () => override.use({ search: {}, navigate: () => {} } as never),
        { wrapper: harness.wrapper },
      );
      expect(result.current.source, `${entity} needs a list source`).toEqual(
        expect.any(Function),
      );
    },
  );
