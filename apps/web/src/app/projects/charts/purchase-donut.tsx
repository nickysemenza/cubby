...
export function PurchaseDonut({
  ...
  const { data, total, netTotal, excludedTotal, purchasesByType } =
    useMemo(() => {
      ...
      // ALL purchases incl. negatives — this is what the center number shows so it
      // reconciles with the StatTile "Total cost".
      const total = sumBy(data, (d) => d.value);
      const netTotal = sumBy(purchases, (p) => p.cost ?? 0);
      ...
      return { data, total, netTotal, excludedTotal, purchasesByType };
    }, [purchases]);
