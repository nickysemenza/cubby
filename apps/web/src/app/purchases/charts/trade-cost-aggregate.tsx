import {
  type CostType,
  costTypeValues,
  type PurchaseTradeCostAggregate,
  type Trade,
...
/**
 * Groups `purchase.analytics`'s `tradeCostMatrix` rows (already one row per
 * trade×costType, server-aggregated) into the same trade-keyed pivot shape
 * `~/app/projects/charts/trade-cost-pivot.ts` builds from raw purchases —
 * kept as a local, `net`-keyed variant here so the analytics view never needs
 * a raw purchase fetch. Negative-net rows are kept (refunds/credits are
 * real); only all-zero trades are dropped.
 */
function buildAggregatePivot(rows: PurchaseTradeCostAggregate[]) {
...
export function TradeBarsAggregate({
  ...
  tradeCostMatrix: PurchaseTradeCostAggregate[];
}) {
 ...
 * are dropped since aggregates carry no per-purchase rows; the click-to-filter
 * affordance is kept). */
