import type { PurchaseCostTypeAggregate } from "@cubby/schemas/project";
...
/**
 * Cost-type breakdown donut sourced from `purchase.analytics`'s `byCostType`
 * aggregate (one grouped SQL sum per cost type) — the analytics-view
 * replacement for `PurchaseDonut`, which summed a raw purchase fetch
...
export function CostTypeDonut({
  ...
  byCostType: PurchaseCostTypeAggregate[];
  height?: number;
        ...
        tooltip={({ datum }) => (
          <ChartTooltip>
            ...
            <div className="mt-1 text-muted-foreground text-xs">
              {datum.data.count} purchase{datum.data.count === 1 ? "" : "s"}
            </div>
          </ChartTooltip>
        )}
        layers={[
