    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          evidence: { type: 'string', description: 'the actual code/output you saw that proves it' },
          suggestion: { type: 'string' },
        },
      },
    },
    laneSummary: { type: 'string', description: '2-3 sentence overall health assessment of this domain' },
  },
}
...
const VERDICT_SCHEMA = {
  ...
  properties: {
    isReal: { type: 'boolean' },
    severity: { type: 'string', enum: ['critical', 'high', 'medium', 'low'] },
    note: { type: 'string', description: 'one sentence: why confirmed or refuted' },
  },
}
...
3. node scripts/check-conventions.mjs
...
If uncertain after reading the code, set isReal=false. Also re-grade severity honestly if confirmed.`,
    { label: `verify:${laneKey}:${f.file.split('/').pop()}`, phase: 'Verify', schema: VERDICT_SCHEMA, model: 'sonnet', effort: 'low' },
  ).then((v) => (v ? { ...f, lane: laneKey, verdict: v } : null))

const laneResults = await pipeline(
  LANES,
  (lane) => agent(lane.prompt, { label: `audit:${lane.key}`, phase: 'Audit', schema: FINDINGS_SCHEMA, model: lane.model }),
  (result, lane) => {
    if (!result) return { lane: lane.key, summary: '(lane failed)', findings: [] }
    ...
    if (lane.key === 'gates') {
      return { lane: lane.key, summary: result.laneSummary || '', findings: fs.map((f) => ({ ...f, lane: lane.key, verdict: { isReal: true, severity: f.severity, note: 'command output (not adversarially verified)' } })) }
    }
    return parallel(fs.map((f) => () => verifyFinding(f, lane.key))).then((vs) => ({
      lane: lane.key,
...
const confirmed = lanes.flatMap((l) => l.findings.filter((f) => f.verdict.isReal).map((f) => ({ ...f, severity: f.verdict.severity })))
const refuted = lanes.flatMap((l) => l.findings.filter((f) => !f.verdict.isReal))
...
LANE SUMMARIES:
${JSON.stringify(lanes.map((l) => ({ lane: l.lane, summary: l.summary })), null, 2)}

CONFIRMED FINDINGS (${confirmed.length}):
${JSON.stringify(confirmed, null, 2)}

REFUTED (for context on what was checked and cleared — do not include as issues):
${JSON.stringify(refuted.map((r) => ({ lane: r.lane, title: r.title, why: r.verdict.note })), null, 2)}
...
return {
  ...
  laneSummaries: lanes.map((l) => ({ lane: l.lane, summary: l.summary, confirmed: l.findings.filter((f) => f.verdict.isReal).length, refuted: l.findings.filter((f) => !f.verdict.isReal).length })),
  synthesis: report,
}
